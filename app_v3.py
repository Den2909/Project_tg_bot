...
STYLE_MODELS = {
    "vangogh": {"path": "checkpoints/style_vangogh_pretrained/latest_net_G.pth", "name": "Ван Гог"},
    "monet": {"path": "checkpoints/style_monet_pretrained/latest_net_G.pth", "name": "Моне"},
    "ukiyoe": {"path": "checkpoints/style_ukiyoe_pretrained/latest_net_G.pth", "name": "Укиё-э"},
    "cezanne": {"path": "checkpoints/style_cezanne_pretrained/latest_net_G.pth", "name": "Сезанн"},
}
...
def process_image(content_path, style_key):
    ...
    content_tensor = torch.from_numpy(content_img).permute(2, 0, 1).float().unsqueeze(0).to(device)
...
def style_transfer_nst_sync(content_path, style_path, output_path, num_steps=1200):
    ...
    for step in range(num_steps):
        ...
        if step % 150 == 0:
            ...
            style_loss *= style_weight
            tv_loss = total_variation_loss(target) * tv_weight
            print(f"Content Loss: {content_loss.item()}, Style Loss: {style_loss.item()}, TV Loss: {tv_loss.item()}")
    ...
    output, _ = upsampler.enhance(img, outscale=1)  # Только резкость, без масштабирования
...
async def style_transfer_nst(content_path, style_path, output_path):
    ...
    await loop.run_in_executor(None, lambda: style_transfer_nst_sync(content_path, style_path, output_path))
...
async def start_command(message: types.Message):
    await message.answer("Привет! Я бот для переноса стиля и улучшения качества.\n"
...
async def style_command(message: types.Message, state: FSMContext):
    ...
    for style_key, style_info in STYLE_MODELS.items():
        keyboard.add(InlineKeyboardButton(style_info["name"], callback_data=f"style_{style_key}"))
...
@dp.callback_query_handler(lambda c: c.data.startswith("style_"), state=StyleTransferStates.waiting_for_style)
async def process_style_selection(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if style_key not in STYLE_MODELS:
        await callback_query.message.answer("Ошибка: стиль не найден. Попробуйте снова с помощью /style.")
        return

    await state.update_data(style_key=style_key)
    await callback_query.message.answer(f"Выбран стиль: {STYLE_MODELS[style_key]['name']}. Теперь отправь контентное изображение.")
    await StyleTransferStates.waiting_for_content.set()
    await callback_query.answer()

# Обработчик контентного изображения для стилизации
@dp.message_handler(content_types=["photo"], state=StyleTransferStates.waiting_for_content)
async def handle_content_photo(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"Обрабатываю изображение в стиле {STYLE_MODELS[style_key]['name']}, пожалуйста, подождите...")

    output_path = await asyncio.to_thread(process_image, content_path, style_key)

    try:
        with open(output_path, "rb") as photo:
            await message.answer_photo(photo, caption=f"Ваше стилизованное изображение в стиле {STYLE_MODELS[style_key]['name']}!")
    finally:
...
async def handle_enhance_photo(message: types.Message, state: FSMContext):
    ...
    try:
        with open(output_path, "rb") as photo:
            await message.answer_photo(photo, caption="Ваше изображение с улучшенным качеством!")
    finally:
        os.remove(content_path)
        os.remove(output_path)
...
async def nst_command(message: types.Message, state: FSMContext):
    await message.answer("Отправьте стилевое изображение (например, картину, стиль которой хотите перенести).")
...
async def handle_nst_style_photo(message: types.Message, state: FSMContext):
    ...
    await message.answer("Теперь отправьте контентное изображение (фото, на которое нужно перенести стиль).")
...
async def handle_nst_content_photo(message: types.Message, state: FSMContext):
    ...
    try:
        with open(output_path, "rb") as photo:
            await message.answer_photo(photo, caption="Ваше стилизованное изображение (NST)!")
    finally:
        os.remove(content_path)
...
if __name__ == "__main__":
    ...
    for style_key, style_info in STYLE_MODELS.items():
        if not os.path.exists(style_info["path"]):
            print(f"Ошибка: Модель для стиля {style_info['name']} не найдена по пути {style_info['path']}")
            exit(1)
