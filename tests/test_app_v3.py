...
def test_load_model(mocker):
    ...
    for style_key in STYLE_MODELS:
        ...
        assert isinstance(model, torch.nn.Module), f"Модель для {style_key} не является torch модулем"
...
def test_process_image(temp_image, mocker):
    ...
    mocker.patch("torch.nn.Module.load_state_dict", return_value=None)
    mocker.patch("torchvision.transforms.ToTensor", return_value=torch.randn(3, 256, 256))
...
def test_style_transfer_nst_sync(temp_image, mocker):
    ...
    mocker.patch("app_v3.get_features", return_value={
